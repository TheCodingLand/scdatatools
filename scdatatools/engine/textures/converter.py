...
def tex_convert(infile: typing.Union[str, Path, io.BufferedIOBase, io.RawIOBase, NamedBytesIO],
                ...
                converter_cli_args: str = "", converter_bin: str = "", overwrite: bool = False) -> Path:
    ...
    tmpin = tempfile.NamedTemporaryFile(
        suffix='.dds' if dds.is_glossmap(in_name) else Path(in_name).suffix, delete=False
    )
    try:
...
class DDSTextureConverter(plugins.P4KConverterPlugin):
    ...
    def convert(cls, members: typing.List['P4KInfo'], path: typing.Union[Path, str], overwrite: bool = False,
        ...
        def _do_convert(tex_in, tex_out):
            try:
                out = tex_convert(
                    infile=tex_in, outfile=tex_out, converter=converter, converter_bin=converter_bin,
                    converter_cli_args=converter_cli_args, overwrite=overwrite
                )
                if replace:
                    tex_in.unlink()
