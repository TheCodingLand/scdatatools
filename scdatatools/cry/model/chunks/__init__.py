...
class IvoCharacterChunkHeader(ctypes.LittleEndianStructure, StructureWithEnums):
    _fields_ = [
    ...
    _map = {
        'type': defs.CharacterChunkHeaderTypes
    }
...
class MtlName(Chunk):
    def __init__(self, header, data):
        super().__init__(header, data)
        self.name = data[:128].decode('utf-8')
...
CHUNK_FOR_TYPE = {
    ...
    defs.ChunkType.BoneNameList: Chunk,
    ...
    defs.ChunkType.SourceInfo: Chunk,
...
def ivocharacter_chunk_from_header(hdr: IvoCharacterChunkHeader, data: (bytearray, bytes)):
    """
    ...
    :return: `Chunk900`
    """
    return CHARACTER_CHUNK_FOR_TYPE[hdr.type].from_buffer(hdr, data)
