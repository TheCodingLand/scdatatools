...
from scdatatools.engine.model_utils import Vector3D, Quaternion, vector_from_csv, quaternion_from_csv
...
PREFAB_OBJECT_TYPES_TO_PROCESS = {
    'AreaBox', 'Brush', 'Decal', 'Designer', 'GeomEntity', 'Group', 'Ledge', 'LightBox', 'Prefab',
    'ProceduralLayoutConnector', 'ProceduralLayoutElementInfoBox', 'ProceduralLayoutSecondaryElement', 'Shape'
    # Ignored Types
    # 'AudioAreaAmbience', 'AudioTriggerSpot', 'EnvironmentProbe', 'PrefabPivot', 'ParticleEffect', 'VisArea'
}
PREFAB_ENTITY_TYPES_TO_PROCESS = {
    'Light', 'ObjectContainer', 'Room'
    # Ignored Types
...
class Prefab:
    def __init__(self, name, guid, properties, library: 'PrefabLibrary', lib_folders='', description='', category='',
                 footprint=''):
        self.name = name
        self.guid = guid
            ...
            for obj in objects:
                if (ot := obj['type']) in PREFAB_OBJECT_TYPES_TO_PROCESS:
                    if ot == 'Prefab':
                        ...
                        yield {'pos_offset': pos_off, 'rotation_offset': rot_off, 'object': obj}
                    elif ot == 'Group':
    ...
    def from_dict(cls, library, prefab_dict) -> 'Prefab':
        prefab = Prefab(prefab_dict['@Name'], _clean_prefab_id(prefab_dict['@Id']),
                        prefab_dict.get('Properties', {}), library,
                        lib_folders=prefab_dict.get('@LibFolders', ''), description=prefab_dict.get('@Description'),
                        category=prefab_dict.get('@Category', ''), footprint=prefab_dict.get('@Footprint', '')
                        )
...
class PrefabLibrary:
    ...
    @classmethod
    def from_prefab_cryxml(cls, manager, path_or_file: typing.Union[str, typing.IO]) \
