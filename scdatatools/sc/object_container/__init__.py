...
class StreamingObjectContainer:
    ...
    def included_objects(self):
        return {
            cid: chunk for cid, chunk in self.chunks.items() if isinstance(chunk, chunks.IncludedObjects)
        }

    @cached_property
    def cryxml_chunks(self):
        return {
            cid: chunk for cid, chunk in self.chunks.items() if isinstance(chunk, chunks.CryXMLBChunk)
        }
...
class ObjectContainer:
    ...
    def _load_soc_xml(self, soc_xml_path):
        ...
        self.tags = [
            self._sc.tag_database.tags_by_guid[tag_id] for tag in oc_etree.findall('.//Tag')
        ...
        def _parse_children(cur_parent, child_containers):
            ...
            for child_elem in child_containers.findall('./Child'):
                ...
                child['tags'] = [
                    self._sc.tag_database.tags_by_guid[tag_id] for tag_id in child.get('tags', "").split(',')
                    if tag_id in self._sc.tag_database.tags_by_guid
                ]

                try:
                    ent_info = self._sc.p4k.getinfo(f'{self._pak_base}/entdata/{child["guid"]}.entxml')
