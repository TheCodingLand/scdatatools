    ...
    def _find_wems_from_hirc_object(self, obj_dict, _searched=None) -> list:
        ...
        if isinstance(obj, hirc_defs.HIRCEvent):
            ...
            for ea_id in event['object'].event_actions:
                wems.update(self._find_wems_from_hirc_object(self.game_objects['event_action'][ea_id],
                                                             _searched=_searched + [obj.id]))
                return list(wems)
        elif isinstance(obj, hirc_defs.HIRCEventAction):
            return self._find_wems_from_hirc_object(self.game_object_for_id(obj.object_id),
                                                    _searched=_searched + [obj.id])
            ...
            for sid in obj.sounds:
                try:
                    wems.extend(self._find_wems_from_hirc_object(self.game_object_for_id(sid),
                                                                 _searched=_searched + [obj.id]))
                except KeyError as e:
                    logger.error(f'Error looking up sound id from HIRCRandom {obj.id} in '
        ...
        if 'hirc' in b.components:
            for o in b.hirc.objects:
                if o.id not in self.game_objects[o.type.name]:
                    ...
                    _cmp_obj(o, name, cmp['object'], cmp['bank'].filename, 'possible game object duplicate')
        self.banks[name] = b
