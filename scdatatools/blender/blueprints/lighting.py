...
def set_linked_light(obj_name, color, temp=None):
    for obj in find_obj_by_name(obj_name, True):
        ...
        if temp: obj["light_link_temp"] = temp
...
def create_light(name, light, light_group_collection, state='defaultState', bone_name='', parent=None,
                 data_dir: Path = None):
    ...
    bulbRadius = bulbRadius * 0  # set to zero for hard IES light edges, increase for softness
    ...
    for key, val in light['EntityComponentLight'].items():
        ...
        light_obj['states'][key] = {
            'color': (
                float(val['color'].get('@r', 1)), float(val['color'].get('@g', 1)), float(val['color'].get('@b', 1))
            ),
        ...
        if ies_group is not None:
            ...
            if ies_group.node_tree is not None:
                ...
                if light_data.type == 'SPOT':
                    ...
                    ies_group.inputs['Scale'].default_value = (spot_size, spot_size, spot_size)
        ...
        if bone_name:
            if helper := parent.get('helpers', {}).get(bone_name.lower(), {}):
                ...
                if 'rotation' in helper:
                    rotation = (
                        helper['rotation']['w'], helper['rotation']['x'], helper['rotation']['y'],
                        helper['rotation']['z']
                    )
    ...
    location = [
        sum(_)
        for _ in zip(location, str_to_tuple(light.get('RelativeXForm', {}).get('@translation', '0,0,0'), float))
    ]
    ...
    if light.get('GeomLink'):
        if light['GeomLink'].get('@SubObjectName'):
            ...
            set_linked_light(light['GeomLink'].get('@SubObjectName'), light_obj.data.color)
