...
class BlueprintGeometry(dict):
    ...
    """
    def __init__(self, blueprint: 'Blueprint', name, geom_file, pos=None, rotation=None, scale=None, materials=None,
                 attrs=None, helpers=None, parent=None):
        super().__init__()
    ...
    def add_instance(self, name, pos, rotation=None, scale=None, materials=None, attrs=None, soc: dict =None):
        if soc is not None:
...
class Blueprint:
    def __init__(self, name, sc: 'StarCitizen', monitor: typing.Callable = None,
    ...
    def get_or_create_geom(self, geom_path: typing.Union[str, Path], parent: BlueprintGeometry = None,
                           create_params: dict = None, sub_geometry: dict = None) -> (BlueprintGeometry, bool):
        """
        ...
        if geom_name not in self.geometry:
            self.geometry[geom_name] = BlueprintGeometry(self, name=geom_name, geom_file=geom_path, **(create_params or {}))
    ...
    def geometry_for_record(self, record: typing.Union[DataCoreObject, Record, GUID, str],
                            base: bool = False) -> typing.Union[dict[BlueprintGeometry], BlueprintGeometry, None]:
        """ Returns all of the `BlueprintGeometry`s associated with a given `record`.

        > If the `record` has not been processed yet, this will at least `process_geometry_resource_params` for the
        > the `record` causing it's geometry to load just in time
        ...
        if guid not in self.record_geometry:
            from scdatatools.sc.blueprints.processors.datacore import process_geometry_resource_params
            record = self.sc.datacore.records_by_guid[guid]
            try:
                geom_component = next(iter(
                    _ for _ in record.properties['Components'] if _.type == 'SGeometryResourceParams'
                ))
            ...
            if isinstance(container, str):
                if '.' in container:
                    ...
                    self.current_container = previous_container.setdefault(
                        'containers', {}).setdefault(container, self._empty_container(attrs))
            ...
            if self._containers_to_process:
                ...
                if container not in self._processed_containers:
                    ...
                    try:
                        ...
                        self.log(f'failed to process object container "{container}"', exc_info=e)
