...
GEOMETRY_EXTENSIONS = ['.cga', '.cgam', '.cgf', '.cgfm', '.skin', '.skinm', '.chr', '.cdf']
    ...
    def __init__(self, chunk_file: typing.Union[str, Path, P4KInfo, P4KExtFile], *args, **kwargs):
        ...
        chunk_headers = [
            self._chunk_header_class.from_buffer(
                self.raw_data, self.header.chunk_hdr_table_offset + (i * ctypes.sizeof(self._chunk_header_class))
            )
        ...
        for h in chunk_headers:
            try:
                ...
                chunk = chunk_from_header(
                    h, self.raw_data, self, fallback_class=self._chunk_header_class.default_chunk_class
                )
                if (e := time.time_ns() - t) >= 2e+7:
                    print(f"{humanize.precisedelta(timedelta(microseconds=e/1e+6), minimum_unit='microseconds')} - {chunk}")
                self.chunks[chunk.id] = chunk
            except Exception as e:
                logger.exception(f'Error processing chunk {h.id} {repr(h)} in {self.filename}', exc_info=e)
...
class GeometryChunkFile(ChunkFile):
    ...
    def __init__(self, chunk_file: typing.Union[str, Path, P4KInfo, P4KExtFile],
                 data_dir: typing.Union[Path, P4KFile] = None, auto_load_mesh: bool = False, *args, **kwargs):
        if isinstance(chunk_file, P4KExtFile):
            chunk_file = chunk_file.p4kinfo
        if isinstance(chunk_file, P4KInfo):
        ...
        if self.mesh_component is not None:
            for chunk in self.mesh_component.chunks.values():
                ...
                if isinstance(chunk, Node):
                    ...
                    self.nodes[chunk.name] = chunk
                    if chunk.parent_id >= 0 and \
                            chunk.name not in (parent := self.chunks[chunk.parent_id]).children:
    ...
    def lods(self):
        ...
        if self._p4kinfo is not None:
            ...
            lods = {
                lod.stem.split('_lod')[-1].split('.')[0]: lod for lod in model_path.parent.glob(lod_search)
            }
...
def load_chunk_file(chunk_file: typing.Union[str, Path, P4KInfo, P4KExtFile], *args, **kwargs) \
