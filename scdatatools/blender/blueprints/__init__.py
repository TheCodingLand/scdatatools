...
from scdatatools.blender.blueprints.hooks import MODEL_IMPORT_HOOK, BLENDER_REGISTER_HOOK, BLENDER_UNREGISTER_HOOK
from scdatatools.blender.blueprints.lighting import create_light
from scdatatools.blender.blueprints.ui import SCImportEntityPanel, SCImportEntityImportedContainersPanel, \
    SCImportEntityAvailableContainersPanel
from scdatatools.blender.utils import (
    remove_proxy_meshes, deselect_all, copy_rotation, normalize_material_name, hashed_path_key,
...
def get_or_create_geometry(geom_file: Path, data_dir: Path = None, bone_names: list = None,
                           helpers: dict = None, has_tints: bool = False,
                           importer: callable = DEFAULT_MODEL_IMPORTER) -> (object, bool):
    """
    ...
    for obj_name in gc['objs']:
        ...
        if obj.type == "MESH":
            ...
            for i, slot in enumerate(obj.material_slots):
                ...
                verts = [v
                         for f in obj.data.polygons if f.material_index == i
                         for v in f.vertices]
                if verts:
...
def create_geom_instance(geom_file: Path, collection=None, location=None, rotation=None, scale=None, bone_name='',
                         instance_name='', parent=None, loc_offset=None, rot_offset=None, *args, **kwargs):
    ...
    if bone_name and parent is not None:
        if helper := parent.get('helpers', {}).get(bone_name.lower(), {}):
            ...
            if 'rotation' in helper:
                rotation = (
                    helper['rotation']['w'], helper['rotation']['x'], helper['rotation']['y'], helper['rotation']['z']
                )
...
class SCBlueprintImporter:
    def __init__(self, scbp_file, data_dir, auto_import_materials=True, remove_physics_proxies=True,
                 auto_remove_proxy_mesh=True, import_lighting=True, model_importer: callable = DEFAULT_MODEL_IMPORTER):
        ...
        def _walk_containers(p, base_name=''):
            for name, container in p.get('containers', {}).items():
                ...
                if name not in self.containers and (
                        container.get('instances', {}) or
                        container.get('lights', {}) or
                        container.get('socs', [])):
        ...
        panels = [type(entity_panel_id, (SCImportEntityPanel, bpy.types.Panel),
                       {"bl_idname": entity_panel_id, "bl_label": self.entity_collection.name, "importer": self})]

        imported_panel_id = f'VIEW3D_PT_SCImpCont_P_{panel_name[-40:]}'
        panels.append(type(imported_panel_id, (SCImportEntityImportedContainersPanel, bpy.types.Panel),
                           {"bl_idname": imported_panel_id, "bl_parent_id": entity_panel_id, "importer": self}))

        available_panel_id = f'VIEW3D_PT_SCAvailCont_P_{panel_name[-39:]}'
        panels.append(type(available_panel_id, (SCImportEntityAvailableContainersPanel, bpy.types.Panel),
                           {"bl_idname": available_panel_id, "bl_parent_id": entity_panel_id, "importer": self}))

        importers[self.entity_collection.name] = {
            "panels": panels,
            "importer": self
        }
        for panel in panels:
    ...
    def _process_geom(self, gc, entity):
        if self.remove_physics_proxies:
            ...
            proxy_objs = [obj_name for obj_name in gc['objs']
                          if obj_name.split('.')[-1].lower().startswith('$physics_proxy')]
            if proxy_objs:
                for obj_name in tqdm.tqdm(proxy_objs, desc='Removing SC physics proxy objects'):
    ...
    def _instance_geom(self, parent, geom_name, inst_name, inst_attrs, **kwargs):
        ...
        new_instance = create_geom_instance(geom_file, location=inst_attrs.get('pos'),
                                            rotation=inst_attrs.get('rotation'),
                                            scale=inst_attrs.get('scale'), bone_name=bone_name,
                                            instance_name=inst_name, data_dir=self.data_dir, parent=parent,
                                            helpers=entity['helpers'], bone_names=self.bp['bone_names'],
                                            importer=self.model_importer, **kwargs)
        ...
        if new_instance is None:
            ...
            logger.error(f'ERROR: could not create instance for '
            ...
            for port_name, props in loadout.items():
                for geom_name in props['geometry']:
                    if geom_name not in self.bp['geometry']:
                        continue
                    inst = self._instance_geom(parent, geom_name, '', {'attrs': {'bone_name': port_name}}, **kwargs)
    ...
    def import_container(self, name, containers=None, skip_geometry=False, geom_only=False, parent=None,
                         loc_offset=None, rot_offset=None):
        self._created_geom = []
        ...
        if (containers == ['all'] or name in containers) and name not in self.imported_containers:
            with log_time(f'Importing {name} container from {self.bp["name"]}', logger.info):
                if not skip_geometry:
                    ...
                    for geom_name in tqdm.tqdm(container_geom, desc=f'Importing {name} geometry', postfix='',
                                               total=len(container_geom), unit='g'):
                        if (entity := self.bp['geometry'].get(geom_name)) is None:
                            logger.error(f'Unknown geometry "{geom_name}"')
                            return None

                        gc, created = get_or_create_geometry(geom_name, self.data_dir,
                                                             self.bp['bone_names'], entity['helpers'],
                ...
                if not geom_only:
                    ...
                    def _load_instances(cont_name, cont):
                        for geom_name, instances in tqdm.tqdm(cont['instances'].items(),
                                                              desc=f'Instancing {cont_name} geometry', postfix='',
                                                              total=len(cont['instances']), unit='g'):
                            for i_name, inst in instances.items():
                                self._instance_geom(cont_root, geom_name, i_name, inst, collection=cont_collection)

                        if self.import_lighting:
                            lights = cont.get('lights', {})
                            for light_group_name, lights in tqdm.tqdm(lights.items(), total=len(lights),
                                                                      desc=f'Creating Lights', postfix='', unit='l'):
                                ...
                                for light_name, light in lights.items():
                                    try:
                                        create_light(light_name, light, light_group_col, parent=cont_root,
        ...
        for child_container in container['containers']:
            ...
            created_geom += self.import_container(child_container, containers,
                                                  skip_geometry=skip_geometry, geom_only=geom_only,
                                                  parent=cont_root)
        return created_geom
    ...
    def import_(self, containers=None, *args, **kwargs):
        with log_time(f'Importing {self.bp["name"]}', logger.info):
            ...
            if self.default_tint_palette:
                load_tint_palette(self.tint_palettes[self.default_tint_palette], self.tint_palette_node_group,
                                  data_dir=self.data_dir)
        ...
        with use_log_file(
            ...
            try:
                importer = SCBlueprintImporter(
                    self.filepath, data_dir=data_dir, auto_import_materials=self.auto_import_materials,
...
def register():
    ...
    for hook_name, hook in plugin_manager.hooks(MODEL_IMPORT_HOOK):
        ...
        model_importer_enums.append(
            (name, name, hook['kwargs'].get('description', ''))
        )
