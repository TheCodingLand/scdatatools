...
class Blender:
    ...
    @argument("list_versions", aliases=["-l"], description="List detected Blender installations.")
    def install_addon(self, version: typing.List[str] = None, list_versions: bool = False):
        if list_versions:
            print("\n".join(f' {k}:\t{v["path"]}'
                            for k, v in available_blender_installations().items() if v['compatible']))
            return
