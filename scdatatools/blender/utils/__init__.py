...
def available_blender_installations(include_paths: typing.List[Path] = None, compatible_only=False) -> dict:
    ...
    if sys.platform == 'win32':
        ...
        include_paths.update(chain(*([b.parent for b in _.rglob(blender)] for _ in Path('/Applications').glob('Blender*'))))
        ...
        for b in ip.glob(blender):
            try:
                versions = [_.split() for _ in subprocess.run(
                                ...
                                shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=5,
                ...
                if versions:
                    ...
                    blender_installs[bv] = {'path': b, 'compatible': compatible, 'pyver': versions[0][1]}
            except (subprocess.CalledProcessError, StopIteration, subprocess.TimeoutExpired):
                continue
